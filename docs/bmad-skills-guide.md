...
> Inicie cada workflow principal em uma conversa nova do Claude Code. Um contexto limpo reduz a mistura entre descoberta, planejamento, implementação e revisão.

## Como invocar

Digite `/` seguido do nome da skill e descreva objetivo, entradas, restrições e resultado esperado:
...
- **Como:** informe o caminho exato e mantenha uma história por sessão do Claude Code.
...
Fonte: catálogo local `_bmad/_config/bmad-help.csv` e metadados em `.claude/skills/`.
