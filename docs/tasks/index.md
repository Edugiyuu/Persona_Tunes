...
| RT-UI-005 | [Add an optional Elizabeth-guided navigation tutorial on first load](./rt-ui-005-elizabeth-navigation-guide.md) | done | 11/11 (100%) | Opt-in prompt, `driver.js` highlight with the popover off, Elizabeth on the left as a four-frame stack with a CSS blink and an unused `speaking` seam, typed Persona dialogue line, 24h `localStorage`. 98 tests pass, lint and build clean, no overflow at 1440 / 768 / 360, and the spotlit entry stays clickable through to `/musics`. Voice lines and lip-sync frames remain a follow-up. |
